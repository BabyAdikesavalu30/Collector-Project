/**
 * Patches Game Screen (/games/patches)
 * Game 3: Piece it together with 15 levels and level selection.
 */
...
import { usePatchesGame } from '../../src/features/games/patches';
...
export default function PatchesGameRoute() {
  ...
  const loadProgress = useCallback(async () => {
    const p = await getGameProgress('patches');
    setProgress(p);
  }, []);
  ...
  const handleBack = useCallback(() => {
    if (Object.keys(placedPieces).length > 0 && !isCompleted) {
      setShowExitDialog(true);
    } else {
      router.back();
    }
  }, [placedPieces, isCompleted, router]);
    ...
    <View style={styles.container}>
      ...
      >
        <PatchesBoard
          ...
          onRemovePlacedPiece={removePlacedPiece}
          onBoardCellPress={handleBoardCellPress}
          trayTitle={gameT.trayTitle}
        />
      </ScrollView>
