...
| **XP & Points Ledger** | `xpStorage` ([`xp.storage.ts`](../../src/features/xp/xp.storage.ts)) | `xpStorage` | `@vigyaan/xp_transactions` | Client sends `dedupeKey`; backend enforces unique constraint on `(user_id, dedupe_key)` |
...
| **Daily Missions** | `getMissionsSnapshot()` ([`missions.engine.ts`](../../src/features/missions/missions.engine.ts)) | `missions.storage.ts` | `@vigyaan/missions_state` | Daily challenge scheduler `/api/v1/missions/daily` |
